...
//
// The tool list is built dynamically per tenant/app: CloudWatch tools are
// always offered (they run under the agent Lambda's own IAM role, not a
// customer-supplied credential), but GitHub tools are only offered when this
...
function buildTools(ctx) {
  ...
  const allSchemas = [
    ...
    },
    {
      ...
      function: {
        ...
        description:
          "Fetch the CloudWatch metric time series (e.g. 5xx count) for the affected service. Use this to see how the error rate trended over time, not just individual log lines.",
        parameters: {
          type: "object",
          properties: {
            windowMinutes: {
    ...
    },
    {
      ...
      function: {
        ...
        description:
          "Fetch the CloudWatch alarm's current state, threshold, and comparison config that fired this incident.",
  ...
  const executors = {
    get_error_logs: (args) => logs.getRecentErrorLogs(ctx.logGroupName, args.windowMinutes || 15),
    get_deployment_logs: (args) => logs.getDeploymentLogs(ctx.logGroupName, args.windowMinutes || 30),
    get_service_metrics: (args) => metrics.getServiceMetrics(ctx.metricNamespace, args.windowMinutes || 30),
    get_alarm_details: () => metrics.getAlarmDetails(ctx.alarmName),
    ...(githubAvailable
      ? {
