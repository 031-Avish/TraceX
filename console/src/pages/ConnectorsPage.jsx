...
const CATEGORIES = ["Observability", "Code", "Notifications"];
...
export default function ConnectorsPage() {
  ...
  const save = async () => {
    ...
    try {
      ...
      showToast(`${selected.name} connected`, "ok");
  ...
  const disconnect = async () => {
    const ok = await confirm(`Disconnect ${selected.name}? Applications using it will fall back to CloudWatch.`, {
    ...
    try {
      ...
      setSelectedType(null);
      load();
      showToast(`${selected.name} disconnected`, "ok");
    } catch (error) {
      showToast(error.message, "error");
    }
  };
  ...
  return (
    <div>
      <div className="page-heading">
        <div>
          <h1>Integrations</h1>
          <p>Connect the tools TraceX can use during an investigation. Nothing is accessed until you connect it here.</p>
        </div>
        ...
        <div className="connector-setup">
          ...
          <div className="setup-form">
            {(selected.fields || []).map((field) => {
              const alreadyConnected = connectors.some((c) => c.type === selected.type);
              return (
                <div className="field" key={field.key}>
                  <label>
                    {field.label}
                    {field.secret && alreadyConnected ? " (leave blank to keep existing)" : ""}
                  </label>
                  {field.type === "select" ? (
          ...
          <section className="catalog-section" key={category}>
            ...
            <div className="connector-grid">
              {CONNECTOR_CATALOG.filter((item) => item.category === category).map((item) => {
                ...
                return (
                  <button
                    className={`connector-tile ${!item.available ? "unavailable" : ""}`}
                    key={item.type}
                    onClick={() => open(item)}
                  >
                    ...
                    <span className={`badge ${connection ? "connected" : "not-connected"}`}>
                      {connection ? "Connected" : item.available ? "Connect" : item.comingSoon || "Coming soon"}
                    </span>
